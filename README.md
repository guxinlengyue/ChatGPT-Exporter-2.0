...
- 📎 可选导出用户上传附件、可见图片和 GPT 生成文件，并生成附件诊断报告
- 🔄 自动清理引用标记，保持内容整洁
- ⏰ 支持定时提醒导出（Chrome 扩展）
- 🎯 智能文件命名（标题 + 对话 ID）
- ⚡️ 优化大量对话的加载性能
...
- 可选择“导出全部”或“选择对话导出”（支持搜索/筛选/日期范围）
- 导出过程中页面会出现一个悬浮进度球（环形进度 + 百分比），可拖动到不遮挡的位置，导出结束后保留可再次点击打开导出面板
...
- 访问 ChatGPT 后，页面右缘（垂直居中）会出现一个圆形悬浮导出按钮
- **点击**按钮，选择导出模式：
  - **个人空间**：导出个人对话
...
**悬浮按钮交互：**
- **拖动**：可拖到页面任意位置，位置自动记忆；拖近左/右边缘会自动吸附
- **贴边半隐藏**：吸附到边缘后空闲 2.5 秒自动半隐藏为侧边把手，鼠标悬停或点按即可展开
- **右键点击**：重置回默认位置
- **导出进度**：以环形进度 + 百分比显示在悬浮球上，具体状态显示在旁边的气泡中
